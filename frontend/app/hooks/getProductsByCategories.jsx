...
export default function useProductsBycategories(
  ...
  const [categoryInsideImg, setCategoryInsideImg] = useState(null);
  const fetchProducts = async (currentOffset = 0, append = false) => {
    ...
    try {
      ...
      if (result.success && Array.isArray(result.product)) {
        setSubCategoryLists(result.subCategoryLists || []);
        setCategoryInsideImg(result.categoryInsideImage || null);
