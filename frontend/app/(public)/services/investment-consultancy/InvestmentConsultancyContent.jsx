...
export default function InvestmentConsultancyContent() {
  const [projects, setProjects] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_BASE}/public/getsPost?slug=investment-consultancy`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        subtitle="Explore our comprehensive investment consultancy services designed to maximize your returns and minimize risks."
        breadcrumbs={[{ label: "Investment Consultancy" }]}
      />
        ...
        <div className="container" style={{ margin: "0 auto", padding: "0 20px" }}>
          {loading ? (
            ...
            <p style={{ color: "#999", textAlign: "center", fontFamily: "'Chivo', sans-serif", fontSize: "18px", padding: "60px 0" }}>
              No investment consultancy projects found.
            </p>
          )}
        </div>
      </section>
