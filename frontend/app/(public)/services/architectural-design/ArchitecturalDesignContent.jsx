...
export default function ArchitecturalDesignContent() {
  const [projects, setProjects] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_BASE}/public/getsPost?slug=architectural-design`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        subtitle="Discover our innovative architectural design solutions that create stunning and functional spaces."
        breadcrumbs={[{ label: "Architectural Design" }]}
      />
        ...
        <div className="container" style={{ margin: "0 auto", padding: "0 20px" }}>
          {loading ? (
            ...
            <p style={{ color: "#999", textAlign: "center", fontFamily: "'Chivo', sans-serif", fontSize: "18px", padding: "60px 0" }}>
              No architectural design projects found.
            </p>
          )}
        </div>
      </section>
