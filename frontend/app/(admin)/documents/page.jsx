...
export default function DocumentPage() {
  ...
  const fetchUsers = async (
    ...
    try {
      const url = `${process.env.NEXT_PUBLIC_API_BASE}/documents/index?page=${page}&pageSize=${pageSize}&searchQuery=${searchQuery}&selectedFilter=${selectedFilter}`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE}/documents/delete/${id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
        ...
        <div className="d-flex gap-2">
          {perms.includes("edit posts") ? (
            <button
              ...
              onClick={() => router.push(`/documents/edit?id=${row.id}`)}
            >
